...
import at.fhv.students.repository.PersonRepository;
...
/**
 * REST controller for managing {@link at.fhv.students.domain.Person}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class PersonResource {
    ...
    public List<Person> getAllPeople(@RequestParam(required = false) String filter) {
        if ("todo-is-null".equals(filter)) {
            log.debug("REST request to get all Persons where todo is null");
            return StreamSupport
                .stream(personRepository.findAll().spliterator(), false)
                .filter(person -> person.getTodo() == null)
